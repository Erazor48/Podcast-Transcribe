...
import { Card, CardContent } from "@/app/components/ui/card"; // Import custom Card components
import { Progress } from "@/app/components/ui/progress"; // Import custom Progress component
import {
  ForwardIcon,
  PlayIcon,
  RewindIcon,
...
const AudioPlayer: React.FC<AudioPlayerProps> = () => {
  ...
  const handlePlayPause = () => {
    if (isPlaying) {
      ...
      setIsPlaying(false);
      // Remettre le son au début lors de la pause
      if (audioRef.current) {
  ...
  useEffect(() => {
    if (audioRef.current && tracks.length > 0) {
      ...
      audioRef.current.currentTime = 0;
      // Réinitialiser l'état immédiatement pour éviter les problèmes de synchronisation
      setCurrentTime(0);
      setProgress(0);
      if (isPlaying) {
        audioRef.current.play();
